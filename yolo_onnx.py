...
class YOLO_OnnxRuntime:
    ...
    ###HL Addition: drawing center dot instead of bounding box###
    def draw_detection_center(self, img: np.ndarray, box: List[float],class_id: int) -> None:
        """
        Draw center of bouding box of detected object on images.
        ...
        # Draw center of bounding boxes as circle on image
        cv2.circle(img, (int(center_x),int(center_y)), radius=5, color=(0, 0, 255), thickness=10)
    ...
    def preprocess(self) -> Tuple[np.ndarray, Tuple[int, int]]:
        ...
        if isinstance(self.input_image, str):
            ...
            self.img = cv2.cvtColor(np.array(self.input_image), cv2.COLOR_RGB2BGR)    
    ...
    def postprocess(self, input_image: np.ndarray, output: List[np.ndarray], pad: Tuple[int, int]) -> np.ndarray:
        ...
        ###HL Addition: lists to store boxes, conf scores, and class_ids after applying NMS###
        filtered_boxes, filtered_conf_scores, filtered_class_ids = [], [], []
        ...
        return input_image, filtered_boxes, filtered_conf_scores, filtered_class_ids
    ...
    def main(self) -> np.ndarray:
        ...
        # Perform post-processing on the outputs to obtain output image along with box, conf score and class ids
